...
def subsample(infile, nseqs, reps):
        ...
        print('getting weights for', infile)
        aln.set_weights()
        print('got weights')
...
def score_sequences(args):
    ...
    #logps = pd.DataFrame(index=df2.index,columns=[f'msa_{i+1}' for i in range(5)] + [f'runtime_msa_{i+1}' for i in range(5)])
        ...
        #with tqdm(total=len(df2)) as pbar:
            for code, group in df2.groupby('code'):
                if args.do_subsampling:
                    ...
                    print(code, group, sequence)
                    matching_files = glob.glob(f'{args.alignments}/{code}_*_MSA.a3m')
                ...
                '''else:
                    matching_files = glob.glob(f'{args.alignments}/{code}_*_MSA_cov75_id90_reduced_subsampled_{i}.a3m')
                    try:
                ...
                for uid, row in group.iterrows():
                    with torch.no_grad():
                        try:
                            ...
                            logps.at[uid, f'msa_{i+1}'] = np.nan
                            logps.at[uid, f'runtime_msa_{i+1}'] = np.nan
                        pbar.update(1)
                    
    logps['msa_transformer_median_dir'] = logps[[f'msa_{i+1}' for i in range(5)]].median(axis=1)
    logps['msa_transformer_mean_dir'] = logps[[f'msa_{i+1}' for i in range(5)]].mean(axis=1)
    logps['runtime_msa_transformer_median_dir'] = logps[[f'runtime_msa_{i+1}' for i in range(5)]].sum(axis=1)
